...
<?php
    ...
    faxes_log ("faxrcvd> Obteniendo informacion del tiff ".obtener_nombre($tiff_file)." CIDNumber: $CIDNumber CIDName: $CIDName DOMAIN: $domain");
    ...
    global $faxes_path;
    $pathDB = createFolder($company_number, $commID, "in", $domain);
    $pdffile= "$faxes_path/$domain/recvd/$pathDB/fax.pdf";

    //El usuario actual es uucp
    // create pdf in new dir
    tiff2pdf_RCVD($tiff_file, $pathDB, $domain);
    // METO EL FAX EN LA BASE DE DATOS?
    fax_info_insert($name_pdf,$modemdev,$commID,$errormsg,$company_name,$company_number,'in',"$domain/recvd/$pathDB");
    faxes_log ("faxrcvd> Se copio el $name_pdf en la ruta $faxes_path/$domain/recvd/$pathDB y se grabo en la BD.");
    /**********************************************
     *         3) ENVIO EMAIL                     *
     **********************************************/

    $destinatario = obtener_mail_destiny($modemdev);
    $arrConfig    = getConfigurationSendingFaxMail($destinatario,$name_pdf,$company_name,$company_number);
