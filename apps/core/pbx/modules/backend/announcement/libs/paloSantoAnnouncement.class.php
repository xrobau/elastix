...
class paloSantoAnnouncement extends paloAsteriskDB{
    ...
    function getAnnouncement($domain=null,$announcement_name=null,$limit=null,$offset=null){
        ...
        if(isset($announcement_name) && $rg_name!=''){
            $where[]=" UPPER(description) like ?";
            $arrParam[]="%".strtoupper($announcement_name)."%";
        }
        if(count($where)>0){
    ...
    function createDialplanAnnouncement(&$arrFromInt){
        ...
        if($arrAnnouncement===false){
            ...
            foreach($arrAnnouncement as $value){
                ...
                $arrExt[]=new paloExtensions($exten, new ext_noop('Playing announcement '.$value['description']),"n","begin");
                ...
                if($allow_skip || $repeat_msg){
                    ...
                    if($allow_skip){
                        $arrExt[]=new paloExtensions("_X", new ext_noop('User skipped announcement'));
                        if ($return_ivr)
                            $arrExt[]=new paloExtensions("_X", new ext_gotoif('$["x${IVR_CONTEXT}" = "x"]', $value['destination'].':${IVR_CONTEXT},return,1'));
                        else 
                            $arrExt[]=new paloExtensions("_X",  new ext_goto($value['destination']));
                    }
                    
                    if($repeat_msg)
                        $arrExt[]=new paloExtensions($repeat_msg,new ext_goto('s,play'));
                } 
                else
                    $arrExt[]=new paloExtensions("_X", new ext_playback($recording_file.',noanswer'));
            
                // if repeat_msg enabled then set exten to t to allow for the key to be pressed, otherwise play message and go
                $exten = $repeat_msg ? 't':'s';
                if($return_ivr){
                    $arrExt[]=new paloExtensions($exten, new ext_gotoif('$["x${IVR_CONTEXT}" = "x"]', $value['destination'].':${IVR_CONTEXT},return,1'));
                    if($allow_skip || $repeat_msg)
                        $arrExt[]=new paloExtensions('i', new ext_gotoif('$["x${IVR_CONTEXT}" = "x"]', $value['destination'].':${IVR_CONTEXT},return,1'));
                } 
                else{
                    $arrExt[]=new paloExtensions($exten, new ext_goto($value['destination']));
                    if($value['allow_skip'] || $repeat_msg)
                        $arrExt[]=new paloExtensions('i', new ext_goto($value['destination']));
                }
