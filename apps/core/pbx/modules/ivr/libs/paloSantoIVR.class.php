...
class paloIvrPBX extends paloAsteriskDB{
    ...
    function getTotalIvr(){
        if($this->validateIvrPBX()==false){
            return false;
        }
            
        $query="select count(id) from ivr where organization_domain=?";
    ...
    function getIvrs($limit=null,$offset=null){
        if($this->validateIvrPBX()==false){
            return false;
        }
        
        $arrParam=array($this->domain);
        $query="select * from ivr where organization_domain=?";
        $arrParam=array($this->domain);
    ...
    function getAllIvrs($id=null,$limit=null,$offset=null){
        $arrParam=array();
        $search=$pagging="";
        if(isset($id) && $id!=""){
            $search="where id=?";
        ...
        $query="select * from ivr $search $pagging";
        $result=$this->_DB->fetchTable($query,true,$arrParam);
        if($result===false){
            $this->errMsg=$this->_DB->errMsg;
            return false;
                ...
                if($context===false){
                    ...
                    if(isset($ivr["directdial"])){
                        if($ivr["directdial"]=="yes")
                            $context->arrInclude=array("from-did-direct-ivr");
                    }
                    $arrFromInt[]="ivr-".$ivr["id"];
                }
