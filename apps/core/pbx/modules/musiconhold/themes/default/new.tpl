...
<table width="100%" border="0" cellspacing="0" cellpadding="5px" class="tabForm" id="files">
    ...
    {else}
        <tr id="test" style="display:none;">
            ...
            <td > <input type="file" name="file[]" style='width=400px'/> <input type="image" class='delete' src='modules/{$MODULE_NAME}/images/remove1.png' title='Remove' /></td>
        </tr>
        {if count($items) eq 0}
            <tr class="content-files" id="1">
                <td width="20%" nowrap> <label for="file">Filename:</label> </td>
                <td ><input type="file" name="file[]" id="file1" /> <input type="image" class='delete' src='modules/{$MODULE_NAME}/images/remove1.png' title='Remove' /></td>
            </tr>
        {else}
            {foreach from=$items item=i}
                ...
                <tr class="content-files" id="{$j}">
                    ...
                    <td> <input type="hidden" name="current_File[]" value="{$i}"> {$i} <input type="image" class='delete'  src='modules/{$MODULE_NAME}/images/remove1.png' title='Remove' /> </td>
                </tr>
            {/foreach}
        {/if}
    {/if}
