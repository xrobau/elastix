...
class AMIClientConn extends MultiplexConn
{
    ...
    private $_response = array();          // Respuesta recibida del último comando
    private $_colaEsperaRespuesta = array();    // Cola de manejadores de respuestas
...
*/
                array_push($this->_response, $paquete);
            } else {
    ...
    {
        return ((count($this->_colaEsperaRespuesta) > 0 && count($this->_response) > 0) 
            || (count($this->_listaEventos) > 0));
    }

    // Procesar un solo paquete de la cola de paquetes
    function procesarPaquete()
    {
        if (count($this->_colaEsperaRespuesta) > 0 && count($this->_response) > 0) {
            $r = array_shift($this->_response);
            $handler = array_shift($this->_colaEsperaRespuesta);
        	call_user_func($handler, $r);
        }

    	if (count($this->_listaEventos) > 0) {
    ...
    private function send_request($action, $parameters=array())
    {
        if (!is_null($this->sKey)) {
            ...
            if (count($this->_colaEsperaRespuesta) > 0)
                return NULL;            	
            else return $this->wait_response();
        } else return NULL;
    }
    ...
    {
        while (!is_null($this->sKey) && count($this->_response) == 0) {
            if (!$this->multiplexSrv->procesarActividad()) {
                usleep(100000);
            }
        }
        if (count($this->_response) > 0) {
        	$r = array_shift($this->_response);
            return $r;
        }
        if (is_null($this->sKey)) {
    ...
    */
    ...
    function enqueue_response_handler($callback)
    {
    	array_push($this->_colaEsperaRespuesta, $callback);
    }
