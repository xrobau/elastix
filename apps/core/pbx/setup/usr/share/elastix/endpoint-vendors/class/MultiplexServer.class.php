...
class MultiplexServer
{
    ...
    function procesarActividad($iMaxTimeout = 1)
    {
        ...
        if (count($listoLeer) == 0 && count($listoEscribir) == 0) return FALSE;

        $iMaxTimeout_sec = floor($iMaxTimeout);
        $iMaxTimeout_usec = (($iMaxTimeout - $iMaxTimeout_sec) * 1000000);
        $iNumCambio = @stream_select($listoLeer, $listoEscribir, $listoErr, 
            (int)$iMaxTimeout_sec, (int)$iMaxTimeout_usec);
        if ($iNumCambio === false) {
            ...
            foreach ($this->_conexiones as $sKey => &$conexion) {
                if (in_array($conexion['socket'], $listoEscribir)) {
                    ...
                    } elseif ($iBytesEscritos == 0) {
                        $this->_oLog->output("ERR: conexión cerrada al escribir datos a ".$conexion['socket']);
                        $this->_cerrarConexion($sKey);
                    } else {
            ...
            // Verificar y remover conexiones con timeout
            foreach (array_keys($this->_listaConn) as $sKey) {
                if ($this->_listaConn[$sKey]->timeout()) {
                    $this->_cerrarConexion($sKey);
                }
            }
            ...
            foreach ($this->_conexiones as $sKey => &$conexion) {
                if (in_array($conexion['socket'], $listoEscribir)) {
                    ...
                    if ($iBytesEscritos === FALSE) {
                        ...
                        $this->_listaConn[$sKey]->actualizarActividad();
                        $conexion['pendiente_escribir'] = substr($conexion['pendiente_escribir'], $iBytesEscritos);
                        $bNuevosDatos = TRUE;                        
                    }
                }
