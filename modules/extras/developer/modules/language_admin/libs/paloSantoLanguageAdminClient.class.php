...
class paloSantoLanguageAdmin {
    ...
    function upload($arrayLangTrasl, $module, $language, $arrayTmp, $arrayUp)
    {   
        global $arrConfig;
        ini_set("soap.wsdl_cache_enabled", "0");
        $client  = new SoapClient($arrConfig['url_webservices']);
        $arrayT = $this->differArrays($arrayTmp, $arrayUp);
        $content = $client->sendTraducciones($module, $language, $arrayT);
        return $content;
    }

    function differArrays($arrayTmp, $arrayUp)
    {
        $result = array();
        $i = 0;
        foreach($arrayUp as $key => $value)
        {
            if($i > 4)
            {
                $keyTmp = $this->obtainsKeys($key);
                $tmp = $arrayTmp[$keyTmp];
                $up  = $arrayUp[$key];
                if(strtolower($tmp) != strtolower($up))
                    $result[$keyTmp] = $value;
            }
            else
                $result[$key] = $value;
    ...
    function obtainsKeys($key)
    {
        ...
        if(substr($newKey,0,4) == "lang")
            $newKey2 = substr($newKey,5,strlen($newKey));
        $newKey2 = str_replace('  ','. ',$newKey2);
        return $newKey2;
    }
